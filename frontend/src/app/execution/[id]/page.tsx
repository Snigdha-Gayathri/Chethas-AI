...
function ExecutionDashboard({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    async function loadAndSubscribe() {
      try {
        ...
        dispatch({ type: "SET_ACTIVE_PHASE", payload: "overview" });
        setLoading(false);
  ...
  const isOverview = state.activePhase === "overview" || !activePhaseObj;
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-80px)] gap-4 animate-in fade-in duration-500">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-4 min-h-0">
        ...
        {/* Center/Right Area */}
        {isOverview ? (
          <div className="lg:col-span-9 h-full overflow-y-auto custom-scrollbar bg-[var(--surface)] border border-white/5 rounded-2xl p-6">
            <OverviewPanel 
              execution={state.currentExecution}
              streamEvents={state.streamEvents}
              onPhaseSelect={(pid) => dispatch({ type: "SET_ACTIVE_PHASE", payload: pid })}
            />
          </div>
        ) : (
          <>
